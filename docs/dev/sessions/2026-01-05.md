...
## Del 6: Node System Removal (Sent Kväll) ✅

### Bakgrund
Node/Node2D-systemet behövde tas bort helt för att slutföra migreringen till Actor-baserad arkitektur.

### Ändringar

**RADERADE Filer:**
- `src/engine/core/Node.h`
- `src/engine/core/Node2D.h`
- `src/engine/core/Node.cpp`
- `src/engine/core/Node2D.cpp`

**NYA Basklasser:**
- `Object` - Abstract root base class
- `ActorObject` - Concrete actor base (replaces Node2D)
- `CharacterActor`, `EnvironmentActor`, `SpriteActor`

**Migrerade Klasser:**
- `Camera2D` : Node2D → ActorObject
- `PhysicsBody` : Node2D → ActorObject
- `Layer` : Node2D → ActorObject
- `KinematicBody` : translate() → setPosition()

**Editor Migrering:**
- `SceneGraphPanel`: renderNodeTree → renderActorTree
- `ViewportPanel`: renderSceneNode borttagen
- `CoreRedirects.h`: Node typedefs borttagna

### Git Commits

6. `1a473eb` refactor(core): remove Node system, migrate to ActorObject
   - Node/Node2D helt borttagna från projekt
   - Nya Object/ActorObject basklasser
   - All kod migrerad till ActorObject

7. `e28d059` feat(editor): add FileBrowser and enhance HierarchyPanel for Actor system
