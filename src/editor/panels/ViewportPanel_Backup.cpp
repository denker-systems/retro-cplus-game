 ...
 * @brief Viewport panel core implementation
 */
...
void ViewportPanel::render() {
    ...
    // Render based on current hierarchy level
    if (m_scene) {
        renderSceneView();
    } else if (m_level) {
        renderLevelView();
        ...
        // Fallback to room preview for backward compatibility
        renderRoomPreview();
    }
...
void ViewportPanel::renderToolbar() {
    ...
    // Grid toggle
    if (ImGui::Checkbox("Grid", &m_showSceneGrid)) {
        // Grid toggled
    }
    ImGui::SameLine();
